...
- For EmerNVS workflows, `emercoin-cli` supports `-v` with `name_show` to
  print only the `value` field from the RPC result. This omits surrounding
  metadata such as ownership and expiration fields and makes the output easier
  to consume directly from scripts and pipelines.

  Example:

  `emercoin-cli -v name_show dns:example`

  The value-only extraction is derived from the historical
  `contrib/emercoin-value.py` helper by Jeff Bouchard, Privateness.Network.
